...
export default function App() {
  ...
  const butterflyPositions = useRef([
...
{showLetter && (
  <div className="absolute inset-0 z-[200] flex items-center justify-center bg-pink-100/30 backdrop-blur-sm p-4">
    ...
    >
      ...
      <div className="overflow-y-auto p-6 pt-10 md:p-10 md:pt-16 text-center custom-scrollbar">
          ...
          <div className="flex flex-col gap-3 mt-4 max-w-md mx-auto">
            {WHOLESOME_REASONS.map((reason, idx) => (
              <motion.div 
                ...
                className="bg-white px-4 py-3 rounded-2xl shadow-sm border border-pink-50 text-pink-500 flex items-center gap-3 text-left"
              >
                <span className="text-base md:text-lg">✨</span>
                <span className="text-xs md:text-sm flex-1">{reason}</span>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      {/* Bottom Accent Bar */}
      <div className="h-2 md:h-3 bg-gradient-to-r from-pink-300 via-rose-400 to-pink-300 shrink-0" />
    </motion.div>
  </div>
)}
      </AnimatePresence>
